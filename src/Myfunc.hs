...
changePly (Mes m) p@(Ply pki' _ _ _ py' px0' px1' pdx') (b@(Bul _ bs' by' bx' bdy' _):bss) bls =
  if (bdy'<0 && by'<=py' && bx'>=px0' && bx'<=px1') 
     ...
     else changePly (Mes m) p bss (bls++[b])
...
changeEnms (Mes m) (e@(Enm eki' _ _ _ ey' ex0' ex1' edx'):es) (b@(Bul _ bs' by' bx' bdy' _):bss) enms bls =
  if (bdy'>0 && by'>=ey' && bx'>=ex0' && bx'<=ex1') 
     ...
     else changeEnms (Mes m) (e:es) bss enms (bls++[b])
                          where dr=if(edx'>0) then 1 else if(edx'<0) then (-1) else 0

normalEnm :: Enm -> Enm 
normalEnm e@(Enm eki' emki' ert' emrt' _ ex0' ex1' edx') =
  if(edx'==0) then
...
changeBuls (Mes m) (b@(Bul _ bs' by' bx' bdy' bdx'):bss) bls =
  let nby = by'+bdy'
   in if (nby>(maxY+bs') || nby<(0-bs'))
         then changeBuls (Mes m) bss bls
         else changeBuls (Mes m) bss (bls++[b{by=nby,bx=bx'+bdx'}])
